...
public class AgentTester {
  ...
  private static final String TASK_LOCOMOTION =
      "sim.task.locomotion(duration = 120; terrain = s.t.downhill(a = 5))";
  ...
  public static void main(String[] args) {
    ...
    Drawer drawer =
        ((Function<String, Drawer>) nb.build("sim.drawer(actions=true; nfc=true; enlargement = 5)"))
            .apply("test");
    ...
    Task<Supplier<EmbodiedAgent>, ?> task =
        (Task<Supplier<EmbodiedAgent>, ?>) nb.build(TASK_BALANCING);
    // read agent resource
    String agentName = args.length >= 1 ? args[0] : "worm-vsr-reactive";
    L.info("Loading agent description \"%s\"".formatted(agentName));
    InputStream inputStream =
        AgentTester.class.getResourceAsStream("/agents/%s.txt".formatted(agentName));
