...
public class NumLeggedHybridModularRobot extends AbstractLeggedHybridModularRobot
    implements NumBrained {

  private static final DoubleRange ANGLE_RANGE =
  ...
  public NumLeggedHybridModularRobot(
      List<Module> modules, NumericalDynamicalSystem<?> numericalDynamicalSystem) {
    super(modules);
        ...
        .mapToInt(
            m ->
                m.trunkSensors().size()
                    ...
                    + m.legChunks().stream().mapToInt(c -> c.jointSensors().size()).sum())
        .sum();
  }
  ...
  public List<? extends Action<?>> act(double t, List<ActionOutcome<?, ?>> previousActionOutcomes) {
    ...
    for (int im = 0; im < modules.size(); im = im + 1) {
      ...
      module
          ...
          .forEach(s -> actions.add(((Sensor<Body>) s).apply(moduleBody.downConnector())));
      module
          .rightConnectorSensors()
          .forEach(s -> actions.add(((Sensor<Body>) s).apply(moduleBody.rightConnector())));
      for (int ic = 0; ic < module.legChunks().size(); ic = ic + 1) {
        ...
        legChunk
            .jointSensors()
            .forEach(s -> actions.add(((Sensor<Body>) s).apply(legChunkBody.joint())));
      }
    }
